# -*- coding: utf-8 -*-
"""
Created on Sun Apr  6 17:44:02 2025
...
def windkessel_ode(t, y, Ts, alpha, q0, R1, R2, C2, L,
    ...
    if Ts + 0.08 < t % (60 / HR) < Ts + 0.12:  # Just after systole
        ...
        dpc_dt += 40.0 * np.exp(15 * (t % (60/HR) - (Ts + 0.10)))
...
def introduce_synthesis_randomness(pc_signal):
    """
    Enhance signal with randomness using wavelet decomposition.
    """
    coeffs = pywt.wavedec(pc_signal, 'db6', level=4)
    coeffs[1:] = [c * 1.2 for c in coeffs[1:]]
    return pywt.waverec(coeffs, 'db6')

def process_synthetic_signal(pc, pp, ppg_signal, fs=125):
    """
    Process and assess the synthetic PPG signal along with peripheral pressure `pp`.
    """
    # Process the synthetic central pressure (pc)
    pc_randomized = introduce_synthesis_randomness(pc)
    pc_filtered = lowpass_filter(pc_randomized, cutoff=8, fs=fs, order=4)

    # Normalize synthetic pc to match the scale of the real PPG signal
    scale_factor = np.max(ppg_signal) / np.max(pc_filtered)
    pc_filtered *= scale_factor

    # Time simulation for synthetic signal
    time_simulated = np.linspace(0, len(pc_filtered) / fs, len(pc_filtered))

    # Normalize real PPG signal
    ppg_signal_normalized = (ppg_signal - np.mean(ppg_signal)) / np.std(ppg_signal)

    # Normalize synthetic pc_filtered for comparison
    pc_filtered_normalized = (pc_filtered - np.mean(pc_filtered)) / np.std(pc_filtered)

    # Process the peripheral pressure (pp) - Apply randomization, filtering, and normalization
    pp_randomized = introduce_synthesis_randomness(pp)
    pp_filtered = lowpass_filter(pp_randomized, cutoff=8, fs=fs, order=4)
    pp_filtered_normalized = (pp_filtered - np.mean(pp_filtered)) / np.std(pp_filtered)

    # Find peaks (systolic peaks in real & synthetic PPG)
    real_peaks, _ = find_peaks(ppg_signal_normalized, distance=50)  # Adjust distance based on HR
    synthetic_peaks, _ = find_peaks(pc_filtered_normalized, distance=50)

    # Compute average cycle duration
    real_cycle_durations = np.diff(real_peaks)
    synthetic_cycle_durations = np.diff(synthetic_peaks)

    # Compare cycle durations
    cycle_mismatch = np.abs(np.mean(real_cycle_durations) - np.mean(synthetic_cycle_durations))
    print(f"Cycle Duration Mismatch: {cycle_mismatch}")

    # Dynamic Time Warping (DTW) to find optimal time shift
    distance, path = fastdtw(ppg_signal_normalized, pc_filtered_normalized)
    index_shifts = [p1 - p2 for p1, p2 in path]
    best_shift = np.median(index_shifts) / fs
    time_simulated_shifted = time_simulated + best_shift  

    # Interpolate the filtered synthetic signal to align with the real PPG signal
    interp_func = interp1d(time_simulated_shifted, pc_filtered, kind='cubic', fill_value="extrapolate")
    pc_filtered_shifted = interp_func(time_simulated)

    # Measure DTW distortion after alignment
    normalized_dtw_distortion = distance / len(ppg_signal_normalized)  # Normalize by signal length
    print(f"Normalized DTW Distortion: {normalized_dtw_distortion}")

    # Compute FFT
    real_fft = np.abs(fft(np.array(ppg_signal_normalized)))  # Convert to numpy array
    synthetic_fft = np.abs(fft(pc_filtered_normalized))

    # Frequency axis
    freqs = fftfreq(len(ppg_signal_normalized), 1/fs)

    # Compare dominant frequencies
    real_dominant_freq = freqs[np.argmax(real_fft[:len(freqs)//2])]
    synthetic_dominant_freq = freqs[np.argmax(synthetic_fft[:len(freqs)//2])]

    print(f"Real Dominant Frequency: {real_dominant_freq} Hz")
    print(f"Synthetic Dominant Frequency: {synthetic_dominant_freq} Hz")
    print(f"Frequency Mismatch: {abs(real_dominant_freq - synthetic_dominant_freq)} Hz")

    return (pc_filtered_shifted, ppg_signal_normalized, best_shift, time_simulated_shifted, 
            pc_filtered_normalized, pp_filtered, pp_filtered_normalized, pp_randomized, pc_filtered, pc_randomized)
...
if __name__=="__main__":
    ...
    PWV = 15.0  # Pulse wave velocity in m/s
    distance = 0.5  # Distance between central and peripheral pressure (meters)
    ...
    pc_filtered_shifted, ppg_signal_normalized, time_shift, time_simulated_shifted, \
    pc_filtered_normalized, pp_filtered, pp_filtered_normalized, pp_randomized, pc_filtered, pc_randomized = \
    ...
    plt.figure(figsize=(12, 5))
    plt.plot(t, pc, label="Original Synthetic PPG", color='gray', linestyle='dashed', alpha=0.7)
    plt.plot(t, pp, label="Original Peripheral Pressure pp(t)", color='blue', linestyle='dotted', alpha=0.7)  # Original pp
    plt.plot(t, pp_filtered, label="Filtered Peripheral Pressure pp(t)", color='g', alpha=0.7)  # Filtered pp
    plt.plot(t, pc_randomized, label="Randomized Synthetic PPG", color='purple', alpha=0.7)
    plt.plot(t, pc_filtered, label="Filtered Synthetic PPG", color='red', linewidth=2)
