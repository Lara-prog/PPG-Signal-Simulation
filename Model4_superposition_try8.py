# -*- coding: utf-8 -*-
"""
Created on Thu Apr 17 14:45:10 2025
...
def variable_heart_rate(t, HR_mean=75, HRV=0.3):  # Reduced HRV from 0.8 to 0.3
    """
...
def cycle_amplitude_correction(t, HR_mean=102.0):
    """
...
def windkessel_ode(t, y, Ts, alpha, q0, R1, R2, C2, L,
    ...
    valve_close_start = Ts + 0.08 * (T / 1.0)  # Move timing later
    valve_close_end = Ts + 0.14 * (T / 1.0)    # Extend the duration
    
    t_cycle = t % T
    if valve_close_start < t_cycle < valve_close_end and t > 0.5:
        # Dramatically increase central pressure notch effect
        dpc_dt -= 20.0 * np.exp(-25 * (t_cycle - valve_close_start))  # Increase from 55 to 80
        dpc_dt += 30.0 * np.exp(35 * (t_cycle - (Ts + 0.09 * (T / 1.0))))  # Increase from 100 to 150
        
        # Matching peripheral pressure notch effect with identical timing
        dpp_dt -= 10.0 * np.exp(-25 * (t_cycle - valve_close_start))  # Increase peripheral even more
        dpp_dt += 15.0 * np.exp(35 * (t_cycle - (Ts + 0.09 * (T / 1.0))))  # Make stronger than central
...
def process_synthetic_signal(pc, pp, ppg_signal, fs=125, skip_cycles=1):
    ...
    if start_idx < min_len:
        ...
        time_simulated_shifted = time_simulated[:min_len] + best_shift - 0.026
